#!/usr/bin/env bash
#
# run-baseline.sh — BASELINE (pre-optimization) performance measurement runner.
#
...
# "benchmarks/results/before-performance-results.json" — the exact file name
# that "benchmarks/generate-diff-report.js" (and
# "tests/performance/compare-results.js") read as the BASELINE input.
#
# It is the mirror-sibling of "benchmarks/run-optimized.sh"; the two scripts are
...
# output as the baseline. The intended workflow is:
#   1. Check out the pre-optimization revision, then run this script to capture
#      "before-performance-results.json".
#   2. Check out the optimized revision, then run "benchmarks/run-optimized.sh"
#      to capture "performance-results.json".
...
#
# Usage:
#   bash benchmarks/run-baseline.sh [options]
#   # or, once marked executable with "chmod +x": ./benchmarks/run-baseline.sh
#
# Options:
...
# OUTPUT_FILE below are the ONLY meaningful difference from run-optimized.sh.
# ---------------------------------------------------------------------------
...
OUTPUT_FILE="${ARTIFACTS_DIR}/before-performance-results.json"
...
usage() {
	printf '%s\n' \
		'Usage: bash benchmarks/run-baseline.sh [options]' \
		'' \
		'Run the BASELINE (pre-optimization) performance measurement: bring up the' \
		...
		'results to benchmarks/results/before-performance-results.json.' \
		'' \
		'Options:' \
...
require_command() {
	if ! command -v "$1" >/dev/null 2>&1; then
		printf '[run-baseline] error: required command "%s" was not found in PATH.\n' "$1" >&2
		exit 1
	fi
}
...
cleanup() {
	...
	if [[ -n "${HOST_WP_CONFIG_BACKUP}" && -f "${HOST_WP_CONFIG_BACKUP}" ]]; then
		...
		printf '[run-baseline] Restored the original src/wp-config.php.\n'
	fi
	...
	if [[ "${keep_up}" == true ]]; then
		...
		printf '\n[run-baseline] --keep-up set; leaving the benchmark stack running.\n'
		printf '[run-baseline] Tear it down later with: %s\n' "${down_hint}"
		...
		printf '\n[run-baseline] Tearing down the benchmark stack and removing its volume...\n'
		compose down -v || true
	else
		printf '\n[run-baseline] Tearing down the benchmark stack...\n'
		compose down || true
	fi
...
ensure_assets_built() {
	if [[ -f "${repo_root}/src/wp-includes/js/wp-emoji-loader.js" ]]; then
		printf '[run-baseline] Built src/ assets present; skipping asset build.\n'
		return 0
	fi
	printf '[run-baseline] src/ assets missing; building dev assets (this may take a few minutes)...\n'
...
# ---------------------------------------------------------------------------
bring_up() {
	printf '[run-baseline] Bringing up the benchmark stack (project "%s", port %s)...\n' "${COMPOSE_PROJECT}" "${BENCH_PORT}"
	if ! compose up -d --wait; then
		printf '[run-baseline] "up -d --wait" was unavailable or failed; retrying with a plain "up -d".\n' >&2
		compose up -d
	fi
}
...
wait_for_http() {
	local attempt=1
	printf '[run-baseline] Waiting for %s to respond...\n' "${BENCH_URL}"
	while [[ "${attempt}" -le "${READINESS_ATTEMPTS}" ]]; do
		if curl -fsS -o /dev/null "${BENCH_URL}"; then
			printf '[run-baseline] Benchmark server is responding (attempt %d/%d).\n' "${attempt}" "${READINESS_ATTEMPTS}"
			return 0
		fi
		printf '[run-baseline] ...not ready yet (attempt %d/%d); retrying in %ss.\n' "${attempt}" "${READINESS_ATTEMPTS}" "${READINESS_INTERVAL}"
		sleep "${READINESS_INTERVAL}"
		attempt=$(( attempt + 1 ))
	done
	printf '[run-baseline] error: %s did not become ready after %d attempts.\n' "${BENCH_URL}" "${READINESS_ATTEMPTS}" >&2
	return 1
}
...
wait_for_db() {
	local attempt=1
	printf '[run-baseline] Waiting for the database to accept connections...\n'
	while [[ "${attempt}" -le "${DB_READINESS_ATTEMPTS}" ]]; do
		if compose exec -T \
			...
			printf '[run-baseline] Database is ready (attempt %d/%d).\n' "${attempt}" "${DB_READINESS_ATTEMPTS}"
			return 0
		fi
		printf '[run-baseline] ...database not ready yet (attempt %d/%d); retrying in %ss.\n' "${attempt}" "${DB_READINESS_ATTEMPTS}" "${DB_READINESS_INTERVAL}"
		sleep "${DB_READINESS_INTERVAL}"
		attempt=$(( attempt + 1 ))
	done
	printf '[run-baseline] error: database did not become ready after %d attempts.\n' "${DB_READINESS_ATTEMPTS}" >&2
	return 1
}
...
ensure_wordpress() {
	...
	if [[ -f "${HOST_WP_CONFIG}" ]]; then
		...
		printf '[run-baseline] Preserved existing src/wp-config.php (restored on teardown).\n'
	fi
	printf '[run-baseline] Writing benchmark wp-config.php (db host "%s", db "%s")...\n' "${DB_HOST}" "${DB_NAME}"
	...
	if wp_cli core is-installed >/dev/null 2>&1; then
		printf '[run-baseline] WordPress already installed; skipping install.\n'
	else
		printf '[run-baseline] Installing WordPress for the benchmark run...\n'
		wp_cli core install \
			--url="${BENCH_URL}" \
			--title="${WP_TITLE}" \
	...
	if wp_cli post list --name=block-image --post_type=post --field=ID 2>/dev/null | grep -q .; then
		printf '[run-baseline] Single-post benchmark fixture already present; skipping.\n'
	else
		printf '[run-baseline] Creating single-post benchmark fixture (/2018/11/03/block-image/)...\n'
		wp_cli post create \
			--post_type=post \
			--post_status=publish \
...
install_mu_plugins() {
	...
	for plugin in server-timing.php clear-cache.php; do
		if [[ ! -f "${src_dir}/${plugin}" ]]; then
			printf '[run-baseline] ERROR: required mu-plugin not found: %s\n' \
				"${src_dir}/${plugin}" >&2
			exit 1
		fi
		cp -f "${src_dir}/${plugin}" "${dest_dir}/${plugin}"
		printf '[run-baseline] Installed mu-plugin: %s\n' "${plugin}"
	done
}
...
run_measurement() {
	...
	printf '[run-baseline] Running the performance suite against %s (label: %s)...\n' "${BENCH_URL}" "${LABEL}"
	node benchmarks/run-benchmark.js --label "${LABEL}" --output "${OUTPUT_FILE}"
}

# ---------------------------------------------------------------------------
# print_summary — report where the baseline artifact landed and the next steps.
# ---------------------------------------------------------------------------
print_summary() {
	printf '\n[run-baseline] Baseline measurement complete.\n'
	printf '[run-baseline] Raw results written to: %s\n' "${OUTPUT_FILE}"
	printf '[run-baseline] Next steps:\n'
	printf '[run-baseline]   1. Check out the optimized revision, then run: bash benchmarks/run-optimized.sh\n'
	printf '[run-baseline]   2. Diff the two runs with:                     node benchmarks/generate-diff-report.js\n'
}
...
while [[ $# -gt 0 ]]; do
	case "$1" in
		...
		-h|--help)
			...
			;;
		*)
			printf '[run-baseline] error: unknown option "%s".\n' "$1" >&2
			usage >&2
			exit 1
			;;
	esac
...
if [[ "${no_up}" == true ]]; then
	printf '[run-baseline] --no-up set; assuming the benchmark stack is already running.\n'
else
