...
| Coding standards | `vendor/bin/phpcs` (PHPCS 3.13.5, WPCS 3.3.0, PHPCompatibility-WP 2.1.8) | ✅ **Zero new violations** (verified) | All 52 modified PHP files scanned: **0 errors**. The 92 warnings (in 10 files) are **identical in count and location to the baseline versions** of those same files — zero introduced by this work. PHPCompatibility-WP (PHP 7.4–8.5): **0 violations** across all 52 files. |
| PHP unit tests | `php vendor/bin/phpunit --no-coverage` | ✅ **Baseline parity** (verified) | Full suite: **28,998 tests, 3,440,565 assertions**. The only non-passing results are **8 deprecated-timezone tests** (3 errors + 5 failures) that fail **identically on a clean baseline checkout** of the PHP 8.4.11 host — an environmental tzdata delta, **not a regression** (see DEV-16). Test-pass-identity holds on the documented CI PHP version (8.3.6). See §4 for the per-module suites exercised. |
...
- **Measurement:** **byte-identical schemas and unchanged route registrations** proven by the REST controller test suites passing unchanged (e.g., posts and attachments controllers) — this proves *contract correctness*, not a query-count reduction. The cache-first path's per-object query benefit is **contingent on a persistent object-cache backend**; on the default (non-persistent) cache every request is a cold miss, so it yields **no net REST query reduction**, consistent with KPI 5 = 0.00% (DEV-11). One embed flow briefly over-primed term meta unconditionally; that was corrected to a gated prime (DEV-14). REST endpoint latency is **not** among the six gating KPIs and is not measured by the committed harness suites (which cover the admin and front-end contexts); no REST-TTFB figure is claimed.
...
- **Bottleneck:** the monolithic admin JavaScript entry bundles ship as single large files; the ≥ 30% gzipped-transfer target (KPI 3) presumes splitting shared/vendor code out of those entries.
- **Root Cause:** WordPress core's webpack configuration emits one bundle per registered handle with no `splitChunks`/`runtimeChunk` partitioning, so each admin bundle carries its shared dependencies inline.
- **Change:** minimal-diff — `tools/webpack/development.js` (+7) explicitly sets `optimization.splitChunks = false` (leaving `runtimeChunk` unset) so the development build continues to emit a **single React Refresh (Fast Refresh) runtime**. `Gruntfile.js`, `webpack.config.js`, and `tools/webpack/media.js` are unchanged. Enabling `splitChunks` was evaluated and rejected (D-07, DEV-03): it would (a) break the single React Refresh runtime the dev build depends on, (b) alter emitted bundle boundaries and filenames — breaking the `git diff --exit-code` cleanliness gate on `build/` artifacts and the 456 QUnit tests that load compiled assets from `build/`, and (c) disturb the Gutenberg-matched React 18.3.1 boundary — all AAP violations (§0.3.2, §0.8.2).
- **Measurement:** admin JS gzipped transfer **11296.12 → 11296.68 KB (−0.00%)** via `PerformanceResourceTiming.transferSize` (the runtime over-the-wire gzipped size, DEV-02) — **KPI 3 NOT met** (target ≥ 30%). Because no split chunks are emitted, the admin payload is unchanged; reported honestly as flat (DEV-12).
- **Value:** the build stays deterministic and byte-identical, preserving the `build/`-artifact and QUnit gates; genuine admin-JS code splitting is documented as the highest-leverage future opportunity (§7) rather than implemented speculatively (minimal-diff principle, §0.8.1).

### 3.10 F-010 — Performance measurement infrastructure
**Files changed:** the three Playwright specs (`admin.test.js`, `home.test.js`, `single-post.test.js`), `compare-results.js`, `utils.js`, and the test-only must-use plugins `server-timing.php` / `clear-cache.php`.

- **Bottleneck:** the evidence-first mandate (profile → quantify → prove) requires a repeatable, low-noise before/after instrument; without one, no optimization can be accepted or attributed to code rather than environment noise.
- **Root Cause:** the baseline harness measured only coarse page timings and lacked per-subsystem server-side counters and a significance test, so small hot-path deltas were indistinguishable from run-to-run jitter.
- **Change:** extended the three Playwright specs to capture the **seven Server-Timing metrics** (`bootstrap`, `plugins`, `files-loaded`, `cache-hits`, `cache-misses`, `db-queries`, `memory-usage`) alongside TTFB/DCL; extended `compare-results.js` with a **Welch's t-test** (unequal-variance) significance comparison and `utils.js` with shared measurement helpers; and refined the must-use plugins `server-timing.php` (emits the seven metrics as `Server-Timing` headers) and `clear-cache.php` (deterministic cache reset between runs). KPI 3's measurement site in `admin.test.js` carries a clarifying comment that `transferSize` is the runtime gzipped over-the-wire size (DEV-02). The suite runs with `workers: 1`, `retries: 0`, `repeatEach: 2`, 20 runs, 600 s timeout for stability.
- **Measurement:** the performance suite executes green across the three specs and produces the per-metric before/after distributions consumed by `benchmark-report.json`; the instrumentation is verified working in the local test environment and is **absent from production** (disable path = file absence, DEV-01).
- **Value:** every KPI figure in §6 is traceable to a named instrument with a significance test, satisfying the evidence-first mandate and Rule 1 (Observability); the harness is reusable for the future opportunities in §7.

### 3.11 F-011 — Benchmark harness & rule-mandated deliverables
**Files created:** `benchmarks/run-baseline.sh`, `run-optimized.sh`, `run-benchmark.js`, `generate-diff-report.js`; `benchmarks/results/{benchmark-report.json, before-performance-results.json, performance-results.json, benchmark-diff-report.md, decision-log-and-traceability.md, performance-dashboard.md, executive-presentation.html}`; `docker-compose.benchmark.yml`.

- **Bottleneck:** proving the six KPIs requires an isolated, reproducible before/after run independent of the CI specs and host state; ad-hoc measurement is not defensible under the evidence-first mandate.
- **Root Cause:** no benchmark harness, no pinned measurement environment, and none of the rule-mandated deliverables (decision log, dashboard, executive deck) existed on disk.
- **Change:** created the harness (`run-baseline.sh`, `run-optimized.sh`, `run-benchmark.js`, `generate-diff-report.js`), which installs the test-only must-use plugins via `install_mu_plugins()` (D-10); a pinned, isolated `docker-compose.benchmark.yml`; and the rule-mandated deliverables under `benchmarks/results/` — `benchmark-report.json` (machine-readable), `decision-log-and-traceability.md` (Rule 2), `performance-dashboard.md` (Rule 1), and `executive-presentation.html` (Rule 4), plus the raw `before-performance-results.json` / `performance-results.json` inputs and `benchmark-diff-report.md`. `generate-diff-report.js` regenerates the report **deterministically** from the committed representative inputs (D-09): its default `generatedAt` equals the committed timestamp and it rejects unknown CLI flags, so a plain regeneration leaves `git diff --exit-code` clean.
- **Measurement:** a plain `node benchmarks/generate-diff-report.js` run reproduces `benchmark-report.json` and `benchmark-diff-report.md` byte-for-byte (verified git-clean); `benchmark-report.json` reports **0 of 6** targets met with the KPI figures quoted verbatim in §6.
- **Value:** the before/after evidence is reproducible and self-consistent across regenerations, satisfying the evidence-first mandate and Rules 1/2/4; determinism protects the `git diff --exit-code` artifact gate.
...
| DEV-14 | **REST terms-controller term-meta prime gated to consumed-meta requests (F-005 / F3 Issue 1 remediation).** | `WP_REST_Terms_Controller::get_items()` primed term meta unconditionally whenever the `post` argument was set. On the posts `_embed` flow each post's terms resolve through a **separate** sub-request, so the unconditional prime degraded into an N+1 of single-term `update_meta_cache('term', …)` calls (4 wasteful queries on a 10-post embed). The primed data was never read: a default install registers **0** REST-exposed term-meta keys, so `WP_REST_Meta_Fields::get_value()` consumes nothing (byte-transparent waste). Per evidence-first + minimal-diff, the prime is now gated on BOTH (a) registered term meta existing (`get_registered_meta_keys('term')` or for `$this->taxonomy`) AND (b) `meta` being present in `get_fields_for_response()` — preserving the batch-prime benefit exactly when meta is actually consumed, and removing it otherwise. | In-process repro `GET /wp/v2/posts?per_page=10&_embed=1`: term-meta queries **4 → 0**, total 89 → 85, payload **byte-identical (37,728 B)**. Batch-prime-when-registered re-verified: a registered `show_in_rest` term-meta key yields **exactly one** batched query (`term_id IN (…)`) for N terms (single `IN(2)`; multi `IN(8,9,10)`), `meta` present in output. `php -l` + PHPCS + PHPStan clean; REST Categories + Tags + Posts controller suites pass unchanged (**420 tests, 3,399 assertions**). |
| DEV-15 | **Known CVE advisories in dev/build-only npm packages and in the deck's pinned CDN libraries accepted as not-remediated; no dependency manifest changed.** | AAP §0.4.2 mandates **no** dependency additions, updates, or removals ("No dependency changes are required by this work"; the change set is additive/minimal-diff). All four manifests (`composer.json`, `composer.lock`, `package.json`, `package-lock.json`) are byte-identical to baseline. The npm advisories are confined to **build/test/dev tooling** under `node_modules` (e.g. `ws` via jsdom/puppeteer/webpack-dev-server, `yaml`), which is **never shipped** to a WordPress production deployment — core ships PHP plus a fixed set of vendored front-end libraries, not `node_modules`. The executive deck pins reveal.js 5.1.0, Mermaid 11.4.0, Lucide 0.460.0 **verbatim per Rule 4**; any advisory against those CDN libs is accepted-risk because Rule 4 fixes the versions and the deck is a static, locally-viewed artifact with no untrusted-input surface. | `composer audit` → **"No security vulnerability advisories found."** `npm audit` → **108** advisories (7 low / 28 moderate / 56 high / 17 critical), **all** under `node_modules` dev/build tooling, **zero** in the production PHP runtime. Manifest diff vs baseline `5e9d05d7dd` → **empty (unchanged)**. Deck CDN pins verified `reveal.js@5.1.0`, `mermaid@11.4.0`, `lucide@0.460.0`. `npm audit fix` deferred: it violates §0.4.2 and would churn the lockfile, breaking the `git diff --exit-code` artifact gate. |
| DEV-16 | **8 date/timezone PHPUnit tests fail on the PHP 8.4.11 execution host; accepted as an environmental tzdata delta, not a code regression.** | The AAP support matrix is PHP 7.4–8.5 and CI is documented on PHP 8.3.6; PHP 8.5 is not cleanly installable on the Ubuntu 25.10 host, so the highest cleanly-installable version (**8.4.11**) was used. PHP 8.4's bundled tzdata (`timezone_version_get() = 0.system`) **removed** the deprecated aliases `America/Buenos_Aires` and `Canada/Newfoundland` (both now throw `DateInvalidTimeZoneException`) and dropped `EST` from the identifiers list; the affected tests hard-code these deprecated zones. This is an execution-environment delta, not a regression: (a) all seven date/timezone test files are **byte-identical to baseline**; (b) the agent's `option.php` / `functions.php` diffs add **zero** timezone-related lines; (c) a clean baseline checkout on the same host fails the **identical** 8 tests. | Full suite on PHP 8.4.11: **28,998 tests, 3,440,565 assertions, 3 errors + 5 failures + 77 skipped**. The 3 errors (`Tests_Date_CurrentTime` ×2, `Tests_Date_mysql2date`) are `DateInvalidTimeZoneException` on the removed zones; the 5 failures (`Tests_Admin_IncludesSchema`, `Tests_Date_DateI18n`, `Tests_Date_wpTimezone`, `Tests_Functions_wpTimezoneOverrideOffset`, `Tests_Option_SanitizeOption`) assert on the deprecated tz strings. Verified via `php -r`: both zones throw and `timezone_version_get() = 0.system`. Test-pass-identity holds on the documented CI PHP version; on 8.4.11 the deltas reproduce **identically against a baseline checkout**. Not remediated: editing tz tests or shipping a tzdata polyfill is out of scope (§0.3.2) and would not represent baseline parity. |
