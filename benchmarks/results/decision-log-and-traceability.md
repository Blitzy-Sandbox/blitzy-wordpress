...
|----------|-------------------------|-----------|------------------------|
| **D-01 — Deferred REST-controller loading.** Defer the 53 REST controller classes that `src/wp-settings.php` otherwise `require_once`s unconditionally (44 endpoint controllers + 5 meta-field controllers + 4 search handlers) so a non-REST front-end request never loads them, backed by an `spl_autoload_register()` safety net keyed to a classmap of exactly those 53 classes. The base REST infrastructure (`rest-api.php`, `class-wp-rest-server.php`, `class-wp-rest-response.php`, `class-wp-rest-request.php`, and the base `class-wp-rest-controller.php`) remains eagerly loaded. | (a) PSR-4 autoloader rewrite — *rejected*: too invasive, breaks the minimal-diff principle and the no-dependency production contract. (b) OPcache preloading only — *rejected*: amortizes parse cost but does **not** reduce the `get_included_files()` count that KPI #6 measures. (c) Broad block-editor / platform / new-7.0 deferral — *rejected as speculative*: those subsystems were not isolated as measured bottlenecks, deferring them would risk the test-pass-identity gate and the security-checkpoint ordering, and the ≥30% files-loaded target (KPI #6) is already met by REST-controller deferral together with the other subsystem work (minimal-diff / evidence-first mandate). | The 53 REST controller classes are `require_once`d on every classic front-end request even though such a request never dispatches a REST route; deferring them is the highest-impact minimal-diff bootstrap reduction and, combined with the rest of the subsystem work, reaches the KPI #6 target. | Load-order regressions if code assumes early REST-controller class availability. Mitigated by (i) deferring **only** on genuine non-REST front-end requests via the `wp_is_rest_request()` predicate — real REST requests still load the controllers eagerly, (ii) an `spl_autoload_register()` safety net registered before `rest_api_init`, so `class_exists()`, direct instantiation, and `WP_Post_Type::get_rest_controller()` continue to resolve any of the 53 classes on demand, and (iii) the full 28,930-test PHPUnit run. |
...
### 3.1 F-001a — REST-controller bootstrap deferral

- **Bottleneck:** `src/wp-settings.php` unconditionally `require_once`s the 53 REST controller classes (44 endpoint controllers + 5 meta-field controllers + 4 search handlers) on every request, including classic front-end page views that never dispatch a REST route and therefore use none of them.
- **Root Cause:** WordPress has no PSR-4 autoloader; the bootstrap `require_once`s the REST controller classes eagerly regardless of request context, so on a non-REST request those files are parsed but never executed.
- **Change:** Predicate-driven deferral in `src/wp-settings.php`. The 53 REST controller classes are collected into a `$wp_rest_controller_classmap` (class ⇒ file, in dependency order so each subclass follows its parent). On a genuine non-REST front-end request — guarded by `defined( 'WP_USE_THEMES' ) && WP_USE_THEMES && ! is_admin() && ! wp_is_rest_request()` — their loading is deferred to the `rest_api_init` hook (priority `PHP_INT_MIN`), while an `spl_autoload_register()` safety net (registered before the hook) resolves any of the 53 classes on demand. Every other context (admin, REST, AJAX, cron, CLI, and the PHPUnit suite) loads them eagerly via the same classmap. The `wp_is_rest_request()` predicate lives in `src/wp-includes/load.php`. Minimal-diff; initialization order relative to security checkpoints is preserved, and the base REST infrastructure stays eagerly loaded.
- **Measurement:** PHP files loaded per front-end request **600 → 417** (aggregate for the optimized front-end runtime) — instrument: `get_included_files()` count. The F-001a change specifically removes the 53 REST controller classes from a non-REST front-end request; the remaining reduction comes from the other subsystem optimizations measured in the same harness.
- **Value:** Contributes to the **30.50% aggregate reduction in files loaded (KPI #6 — PASS, ≥30%)**. Fewer parsed files also lower peak memory and shorten bootstrap time, contributing to the memory (KPI #4) and TTFB (KPI #1) wins. Beneficiaries: every classic front-end request that does not dispatch a REST route, on every WordPress site.
...
| **Bootstrap deferral narrowed to REST controllers only** | The AAP framing of F-001 names block-editor (~73), REST-controller (~58), and new-7.0 (~23) files as the highest-deferral-potential categories, implying broad context-aware deferral of all three. | `src/wp-settings.php` defers **only** the 53 REST controller classes (44 endpoints + 5 meta-field controllers + 4 search handlers), predicate-gated by `wp_is_rest_request()` and backed by an `spl_autoload_register()` safety net; block-editor and new-7.0 subsystems remain eagerly loaded. | The same AAP forbids **speculative optimization** and mandates the **minimal-diff / evidence-first** principle (§0.3.2, §0.7.2, §0.8.1). Broad block-editor / new-7.0 deferral was not isolated as a measured bottleneck, would materially risk the test-pass-identity gate (28,930 PHPUnit) and the security-checkpoint ordering, and is **unnecessary** because the ≥30% files-loaded target (KPI #6) is already met by REST-controller deferral combined with the other subsystem work. Broadening deferral is left as a possible future exploration — pending measurement that isolates those subsystems as bottlenecks — rather than forced now. |
| **Google Fonts stylesheet — SRI intentionally omitted (`executive-presentation.html` L20)** | M8 / checkpoint security finding: add Subresource Integrity + `crossorigin` to the pinned CDN CSS/JS resources (cited lines 20, 23, 906–908). | reveal.css (L23) is **inlined** verbatim from the pinned build; the three executable libraries — reveal.js, Mermaid, and Lucide (L906–908) — carry SHA-384 `integrity` + `crossorigin="anonymous"` + `referrerpolicy="no-referrer"`. The Google Fonts stylesheet (L20) keeps its HTTPS link with the recommended `preconnect` + `crossorigin`, and SRI is deliberately **not** applied. | The Google Fonts CSS API returns **user-agent-dependent (dynamic)** CSS, so a static integrity hash would break rendering across browsers — the exact catastrophic block SRI is meant to prevent. Fonts are **non-executable** (outside SRI’s code-tampering threat model), load over HTTPS, and **degrade gracefully** to the theme’s system `sans-serif`/monospace fallbacks. Self-hosting all eight Google-served subsets (~490 KB base64, mostly unused Cyrillic/Greek/Vietnamese/symbols) would violate the minimal-diff principle for a documentation artifact, aligning with the reviewer’s "applicable CDN CSS/JS resources" qualifier. |
...
graph TD
    ...
    A["HTTP Request (any context)"] --> B["wp-settings.php<br/>eager require/include bootstrap chain"]
...
The diagram *Runtime Architecture — After* shows the target-state hot path: a context-aware, deferred runtime. On a genuine non-REST front-end request the bootstrap defers the 53 REST controller classes to `rest_api_init` behind an `spl_autoload_register()` safety net (real REST requests, admin, AJAX, cron, and CLI still load them eagerly), the hook dispatcher uses an arity decision tree with an empty-callback fast path, the query layer batch-primes metadata and terms in bulk, the object cache exposes multi-get/set with granular invalidation and per-group counters, and Server-Timing emits seven metrics in the test environment. All public contracts are unchanged, so the response output is byte-identical.

```mermaid
graph TD
    %% Diagram: Runtime Architecture — After (context-aware, deferred runtime)
    %% Legend: diamond = request-context predicate; only a non-REST front-end request
    %% defers the 53 REST controller classes (every other context loads eagerly);
    %% all public contracts (signatures, hooks, REST schemas, wp.* API) are preserved.
    A2["HTTP Request"] --> CTX{"Non-REST front-end?<br/>WP_USE_THEMES &amp;&amp; ! is_admin()<br/>&amp;&amp; ! wp_is_rest_request()"}
