...
class WP_Styles extends WP_Dependencies {
	...
	/**
	 * Per-handle cache of fully-qualified CSS URLs generated by _css_href().
	 *
	 * Within a single request the source URL, version string, base_url,
	 * content_url, and default_version for a given handle do not change,
	 * so the expensive URL assembly (string concatenation, rawurlencode,
	 * esc_url, and the style_loader_src filter) can be performed once and
	 * the result reused on any subsequent call with the same handle.
	 *
	 * Cleared in reset() so that concatenation cycles start fresh.
	 *
	 * @since 7.0.0
	 *
	...
	public function do_item( $handle, $group = false ) {
		...
		/*
		 * Pre-compute escaped attribute values that are reused across the
		 * main <link> tag and the optional RTL <link> tag.  This avoids
		 * redundant esc_attr() calls when the RTL branch is taken.
		 */
		$esc_handle = esc_attr( $handle );
		$esc_media  = esc_attr( $media );
		$title_attr = $title ? sprintf( " title='%s'", esc_attr( $title ) ) : '';
	...
	public function _css_href( $src, $ver, $handle ) {
		/*
		 * Return the cached URL when available.  Within a single request
		 * the source, version, base_url, content_url, default_version,
		 * and args for a given handle do not change, so the full URL
		 * assembly (string concatenation, rawurlencode, esc_url, and the
		 * style_loader_src filter) only needs to run once per handle.
		 */
		if ( isset( $this->href_cache[ $handle ] ) ) {
			return $this->href_cache[ $handle ];
		}

		/*
		 * Determine whether $src is already an absolute URL.  Use native
		 * string functions instead of preg_match() to avoid regex engine
		 * overhead on this hot path — the original pattern was
		 * '|^(https?:)?//|' which matches http://, https://, or //.
		 */
		if ( ! is_bool( $src )
			&& ! str_starts_with( $src, '//' )
	...
	public function reset() {
		...
		// Clear the per-handle URL cache so a fresh concatenation cycle
		// does not carry over stale results.
		$this->href_cache = array();
	}
