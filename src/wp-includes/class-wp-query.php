...
class WP_Query {
	...
	/**
	 * Maximum number of entries retained in the request-level query result memo.
	 *
	 * Bounds the memory used by the within-request memoization store (see
	 * self::$query_result_memo) so that pages issuing a very large number of
	 * distinct cacheable queries cannot grow the memo without limit. A single
	 * page realistically resolves far fewer than this many distinct post
	 * queries, so the bound never reduces effectiveness in practice while
	 * keeping worst-case memory predictable. Eviction is FIFO (oldest first).
	 *
	 * @since 7.0.0
	 * @var int
	 */
	const QUERY_MEMO_LIMIT = 100;

	/**
	 * Request-level memoization store for fully-resolved post-query results.
	 *
	 * Keyed by the cache key produced by ::generate_cache_key(), each entry
	 * stores the resolved result payload (`posts`, `found_posts` and
	 * `max_num_pages`) alongside the `last_changed` salt that was in effect
	 * when the entry was written. Because the entry is only considered a hit
	 * when the stored salt matches the current `posts` (and `terms`) last
	 * changed value, the memo is transparently invalidated whenever the
	 * underlying data mutates within the request, mirroring the invalidation
	 * semantics of the persistent `post-queries` object-cache group.
	 *
	 * This complements the persistent cache: when the same query arguments are
	 * resolved more than once in a single request, the result is served from
	 * process memory without re-serializing the cache key or performing another
	 * (potentially remote) object-cache lookup, and without re-running the SQL.
	 * The store is shared across WP_Query instances (hence static) and bounded
	 * by self::QUERY_MEMO_LIMIT.
	 *
	 * @since 7.0.0
	 * @var array<string, array{data: array, salt: string}>
	 */
	private static $query_result_memo = array();
	...
	public function get_posts() {
		...
		if ( $query_vars['cache_results'] && $id_query_is_cacheable ) {
			...
			if ( null === $this->posts ) {
				/*
				 * Consult the request-level memo before the (potentially remote)
				 * object-cache lookup. On an object-cache hit the memo is warmed
				 * so subsequent identical queries in this request resolve from
				 * process memory. Invalidation is handled by the `$last_changed`
				 * salt inside ::get_query_cache().
				 */
				$cached_results = $this->get_query_cache( $cache_key, $last_changed );
		...
		if ( 'ids' === $query_vars['fields'] ) {
			...
			if ( $query_vars['cache_results'] && $id_query_is_cacheable ) {
				...
				$this->set_query_cache( $cache_key, $cache_value, $last_changed );
			}
	...
	/**
	 * Retrieves a cached post-query result, preferring the request-level memo.
	 *
	 * The lookup first consults the static, request-scoped memoization store
	 * (see self::$query_result_memo). On a memo miss it falls back to the
	 * persistent `post-queries` object-cache group via wp_cache_get_salted();
	 * a value found there is copied into the memo so that any further identical
	 * query in the same request is served from process memory without another
	 * (potentially remote) object-cache lookup. Only cacheable queries reach
	 * this method because the caller gates on `cache_results` and query
	 * cacheability, so no additional gating is performed here.
	 *
	 * The returned payload has the same shape written by ::set_query_cache():
	 * an array with `posts`, `found_posts` and `max_num_pages` keys, or false
	 * when no valid (unexpired, unchanged) cache entry exists.
	 *
	 * @since 7.0.0
	 *
	 * @param string          $cache_key    Cache key generated by ::generate_cache_key().
	 * @param string|string[] $last_changed The `posts` (and `terms`) last changed value(s)
	 *                                       used as the cache salt for invalidation.
	 * @return array|false The cached result payload, or false if not present or stale.
	 */
	private function get_query_cache( $cache_key, $last_changed ) {
		$salt = is_array( $last_changed ) ? implode( ':', $last_changed ) : (string) $last_changed;

		if ( isset( self::$query_result_memo[ $cache_key ] )
			&& self::$query_result_memo[ $cache_key ]['salt'] === $salt
		) {
			return self::$query_result_memo[ $cache_key ]['data'];
		}

		$cached_results = wp_cache_get_salted( $cache_key, 'post-queries', $last_changed );

		if ( $cached_results ) {
			// Warm the request-level memo from the persistent cache hit.
			$this->memoize_query_result( $cache_key, $cached_results, $salt );
		}
	...
	/**
	 * Stores a resolved post-query result in both cache layers.
	 *
	 * Writes the payload to the persistent `post-queries` object-cache group
	 * (preserving the existing invalidation behaviour driven by `$last_changed`)
	 * and mirrors it into the request-level memo so that a subsequent identical
	 * query in the same request avoids re-running the SQL and re-serializing the
	 * cache key. This is a thin wrapper around wp_cache_set_salted(): it does not
	 * change what is cached, the cache group, or when the entry expires.
	 *
	 * @since 7.0.0
	 *
	 * @param string          $cache_key    Cache key generated by ::generate_cache_key().
	 * @param array           $cache_value  Result payload with `posts`, `found_posts`
	 *                                       and `max_num_pages` keys.
	 * @param string|string[] $last_changed The `posts` (and `terms`) last changed value(s)
	 *                                       used as the cache salt for invalidation.
	 * @return bool True on successful object-cache write, false on failure.
	 */
	private function set_query_cache( $cache_key, $cache_value, $last_changed ) {
		$salt = is_array( $last_changed ) ? implode( ':', $last_changed ) : (string) $last_changed;

		$this->memoize_query_result( $cache_key, $cache_value, $salt );

		return wp_cache_set_salted( $cache_key, $cache_value, 'post-queries', $last_changed );
	}

	/**
	 * Writes an entry into the bounded request-level query result memo.
	 *
	 * Maintains FIFO ordering and evicts the oldest entry once the store grows
	 * beyond self::QUERY_MEMO_LIMIT, keeping worst-case memory usage bounded for
	 * requests (or long-lived CLI/test processes) that resolve a large number of
	 * distinct queries. Re-writing an existing key refreshes its position so it
	 * is treated as most-recently written.
	 *
	 * @since 7.0.0
	 *
	 * @param string $cache_key   Cache key generated by ::generate_cache_key().
	 * @param array  $cache_value Result payload to memoize.
	 * @param string $salt        Flattened `last_changed` salt captured at write time.
	 */
	private function memoize_query_result( $cache_key, $cache_value, $salt ) {
		// Remove any existing entry so the refreshed value moves to the end (FIFO recency).
		unset( self::$query_result_memo[ $cache_key ] );

		self::$query_result_memo[ $cache_key ] = array(
			'data' => $cache_value,
			'salt' => $salt,
		);

		if ( count( self::$query_result_memo ) > self::QUERY_MEMO_LIMIT ) {
			// Evict the oldest entry to keep the memo bounded.
			array_shift( self::$query_result_memo );
		}
	}
