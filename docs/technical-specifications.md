...
> **Final Delivery Status — read this first.** This document is the *full-project* technical specification / Agent Action Plan for the WordPress 7.0.0-alpha performance refactor; it describes the **target** architecture across all features (F-001–F-011). This is the **final delivery state**: the full optimization change set has been implemented across **51 core source files** under `src/` (F-001 → F-008), plus the build entry (`tools/webpack/development.js`), the F-010 measurement infrastructure, and the F-011 benchmark harness and Rule deliverables. Five in-scope files were intentionally left unchanged where no byte-identical-safe optimization exists — `default-filters.php` (F-001), `class-wp-query.php` (F-002), `general-template.php` (F-004), `script-loader.php` (F-006), and `ajax-actions.php` (F-008); the rationale for each is recorded in `benchmarks/results/decision-log-and-traceability.md` (deviations DEV-04/05/06).
>
> **Performance KPIs have been measured** and the reproducible report (`benchmarks/results/benchmark-report.json`, produced by genuine before/after harness runs of 10 iterations × 2 repetitions) records **zero of six aggressive targets met**, with three metrics showing real, statistically-significant movement toward target and three flat: front-end TTFB −10.06% (390.60 → 351.30 ms, target ≥20%), PHP files loaded −12.58% (493 → 431, target ≥30%), and PHP memory −5.31% (6.77 → 6.41 MB, target ≥10%) improved but fell short; admin DOMContentLoaded −0.17% (1026.75 → 1028.50 ms, target ≥15%), admin JS gzipped transfer −0.00% (11296.12 → 11296.68 KB, target ≥30%), and DB queries 0.00% (21 → 21, target ≥15%) are flat. The gaps are bounded by hard byte-identical-output and full test-suite parity gates and are documented as accepted partials with AAP citations in the decision log (DEV-02, DEV-11, DEV-12, DEV-13). All measurement and reporting artifacts — the `server-timing.php` metric emitter, the performance-spec and `compare-results.js` extensions, the benchmark harness (`run-baseline.sh` / `run-optimized.sh` / `run-benchmark.js` / `generate-diff-report.js` / `benchmark-report.json`), and the Rule deliverables (`performance-dashboard.md`, `decision-log-and-traceability.md`, `executive-presentation.html`) — are **delivered**. The report regenerates deterministically from the committed genuine measurement inputs via `SOURCE_DATE_EPOCH` (decision log DEV-08).
...
- **Measure-First Rule** — Use `tests/performance/` Playwright suite (TTFB, LCP, Server Timing metrics), `SAVEQUERIES` constant for DB query counting, `memory_get_peak_usage()` for PHP memory, `get_included_files()` for file count, and runtime browser-measured gzipped transfer (`PerformanceResourceTiming.transferSize`) for admin JS size (see decision log DEV-02)
...
- `src/wp-includes/rest-api/endpoints/*.php` — N+1 query elimination across the 45 endpoint controllers; 10 controllers are optimized this phase (attachments, block-types, comments, post-types, posts, search, settings, taxonomies, terms, users), with the remaining 35 documented as a future phase
...
**Test & Measurement-Infrastructure Updates** (full-project scope; all files below are delivered — the three performance specs, `compare-results.js`, `utils.js`, and the `server-timing.php` / `clear-cache.php` must-use plugins):
...
Per the Observability implementation rule, performance instrumentation ships together with the optimization work rather than as a follow-up. **Diagram 4 — "Observability: Before vs After"** contrasts the baseline observability signals with the seven Server-Timing metrics delivered in the measurement layer, so the reader sees both the prior state and the delivered state.

**Diagram 4 — Observability: Before vs After (Server-Timing instrumentation).**

```mermaid
graph LR
    %% LEGEND: left subgraph = BEFORE (baseline signals); right subgraph = AFTER (the 7 delivered Server-Timing metrics).
    %% Arrows show which baseline signal each delivered metric derives from. Test/development-only; disabled in production by file absence.
    ...
    subgraph AfterState["After — 7 Delivered Server-Timing Metrics"]
        A1["bootstrap"]
        A2["plugins"]
        A3["files-loaded"]
        A4["cache-hits"]
...
*Legend:* the left **Before** subgraph lists the baseline signals; the right **After** subgraph lists the seven delivered Server-Timing metrics; solid arrows show which baseline signal each metric derives from and the dotted `preserved` edge indicates `WP_DEBUG_LOG` continues to function unchanged. Both states are shown — this is a before/after diagram.

The `tests/performance/wp-content/mu-plugins/server-timing.php` must-use plugin emits these seven metrics — `bootstrap` (time from `$timestart` to end of `wp-settings.php`), `plugins` (time spent loading plugins), `files-loaded` (count from `get_included_files()`), `cache-hits`/`cache-misses` (from the new per-group `WP_Object_Cache` internal counters), `db-queries` (from `$wpdb->num_queries`/`SAVEQUERIES`), and `memory-usage` (from `memory_get_peak_usage()`). These metrics flow through the existing Playwright `metrics.getServerTiming()` infrastructure without new test framework tooling, and the plugin is test/development-only — its off state is the physical absence of the file, so it is never present in a production deployment. The `benchmarks/results/performance-dashboard.md` dashboard and the KPI slides of `benchmarks/results/executive-presentation.html` visualize exactly these seven metrics. **Status:** `server-timing.php`, the benchmark harness, and these reporting artifacts are **delivered**; the benchmark runners install the must-use plugins into the (gitignored) runtime `mu-plugins` directory via an idempotent `install_mu_plugins()` step (decision log D-10), and the report regenerates deterministically from committed genuine measurement inputs (DEV-08).

### 0.4.4 Performance Target Architecture

The six performance targets, their measurement instruments, and their **measured** results are summarized below. All figures are sourced verbatim from the reproducible `benchmarks/results/benchmark-report.json` (which regenerates deterministically from committed genuine measurement inputs — decision log DEV-08). Consistent with the evidence-first mandate, each figure is a measured before→after delta, not a projection.

| Metric | Target | Measured Result | Measurement Instrument |
|--------|--------|-----------------|------------------------|
| Front-end TTFB (uncached) | ≥20% reduction | ❌ **−10.06%** (390.60 → 351.30 ms) — real gain, below target | `tests/performance/specs/home.test.js` |
| Admin DOMContentLoaded | ≥15% reduction | ❌ **−0.17%** (1026.75 → 1028.50 ms) — flat | `tests/performance/specs/admin.test.js` |
| Admin JS transfer size (gzipped) | ≥30% reduction | ❌ **−0.00%** (11296.12 → 11296.68 KB) — flat | Runtime browser-measured gzipped transfer (`PerformanceResourceTiming.transferSize`); DEV-02 |
| PHP memory per front-end request | ≥10% reduction | ❌ **−5.31%** (6.77 → 6.41 MB) — real gain, below target | Server-Timing `wp-memory-usage` (`memory_get_peak_usage()`) |
| DB queries per front-end page load | ≥15% reduction | ❌ **0.00%** (21 → 21) — flat, already at batched floor | Server-Timing `wp-db-queries` (`SAVEQUERIES` / `$wpdb->num_queries`) |
| PHP files loaded per front-end request | ≥30% reduction | ❌ **−12.58%** (493 → 431) — real gain, below target | Server-Timing `wp-files-loaded` (`get_included_files()`) |

**Zero of six aggressive targets are met**, per the genuinely-measured report. Three metrics show real, statistically-significant improvement below target — PHP files loaded −12.58% (62 fewer files/request), front-end TTFB −10.06%, and PHP memory −5.31% — and three are flat: admin DOMContentLoaded and admin JS transfer (admin bundles load eagerly by design; webpack `splitChunks` cannot touch the Grunt-uglified admin JS — decision log DEV-02/DEV-03/DEV-12), and DB queries (already at the batched WordPress 6.1+ floor — DEV-11). All gaps are accepted partials bounded by the hard byte-identical-output and full test-suite-parity gates, with AAP citations in the decision log (DEV-11/DEV-12/DEV-13). REST endpoint latency is not among the six gating KPIs and is not measured by the committed harness suites; no REST-TTFB figure is claimed. The aggregate before/after summary is recorded in `benchmarks/results/benchmark-report.json`.
...
> **Final delivery note.** The tables in this section are the *full-project* transformation plan: the **Transformation** column states each file's mode (UPDATE/CREATE). This is the final delivery state — the full change set is implemented across 51 core source files plus the build entry, F-010 infrastructure, and the F-011 benchmark harness and Rule 1/2/4 deliverables (`performance-dashboard.md`, `decision-log-and-traceability.md`, `executive-presentation.html`, `benchmark-report.json`). Five in-scope files are intentionally unchanged with documented rationale (DEV-04/05/06). Any KPI thresholds embedded in the "Key Changes" cells are **targets**; the genuinely-measured results (0 of 6 aggressive targets met — three real improvements below target, three flat) are recorded in §0.4.4 and in `benchmarks/results/benchmark-report.json`.
...
Beyond the five primary controllers above (posts, comments, terms, users, attachments), five additional controllers received the same cache-first batch-priming treatment — block-types, post-types, search, settings, and taxonomies — for **10 optimized of the 45 total** endpoint controllers. The remaining 35 lower-traffic controllers are a documented future phase. All REST routes and request/response schemas remain byte-identical to baseline.
...
| Admin JS transfer size (gzipped) | Runtime browser-measured gzipped transfer (`PerformanceResourceTiming.transferSize`) | ≥30% reduction |
